...
CanvasRenderingContext2D.prototype.fillTextVerticalRaw = function (text, x, y) {
  var a = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : -Math.PI / 2;
  var ctx = this;
  ctx.save();
  ctx.translate(x, y);
  ctx.rotate(a);
...
CanvasRenderingContext2D.prototype.fillTextVertical = function (text, x, y) {
  ...
  arrText.forEach(function (letter, index) {
    // 确定下一个字符的纵坐标位置
    var letterWidth = arrWidth[index]; // 是否需要旋转判断

    var code = letter.charCodeAt(0);

    if (code <= 256) {
      context.translate(x, y); // 英文字符，旋转90°
    ...
    context.fillText(letter, x, y); // 旋转坐标系还原成初始态

    context.setTransform(1, 0, 0, 1, 0, 0); // 确定下一个字符的纵坐标位置
